...
export default function SellerProductVariantCreatePage() {
  ...
  return (
    <section className="min-h-screen bg-[#f7f5f4] px-4 py-6 text-[#1b1c1c] lg:px-7">
      <form onSubmit={submit} className="mx-auto max-w-[1680px]">
        <header className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex items-center gap-3"><Link to="/seller/products" className="h-10 rounded-lg border border-[#e3beb6] px-4 py-2 text-sm font-semibold text-[#b42318]">Sản phẩm đã lưu</Link><Link to="/seller/products" className="h-10 px-3 py-2 text-sm font-semibold text-[#b42318]">Hủy</Link><button className="h-10 rounded-lg bg-[#b42318] px-5 text-sm font-semibold text-white shadow-lg" type="submit">Lưu sản phẩm</button></div>
        </header>
        ...
        <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-12">
          <div className="space-y-5 lg:col-span-9">
            <section className="rounded-xl border border-[#e3beb6] bg-white p-6 shadow-sm">
              <h2 className="mb-5 text-lg font-bold">Thông tin cơ bản</h2>
              <div className="space-y-5">
                <Field label="Tên sản phẩm" required><input value={form.name} onChange={(event) => updateForm('name', event.target.value)} className="h-12 w-full rounded-lg border border-[#e3beb6] bg-[#fbf9f9] px-4 text-sm outline-none focus:border-[#ee4d2d]" /></Field>
                <Field label="Danh mục sản phẩm" required><select value={form.category} onChange={(event) => updateForm('category', event.target.value)} className="h-12 w-full rounded-lg border border-[#e3beb6] bg-[#fbf9f9] px-4 text-sm">{categories.map((category) => <option key={category}>{category}</option>)}</select></Field>
                <Field label="Mô tả chi tiết"><textarea value={form.description} onChange={(event) => updateForm('description', event.target.value)} className="h-28 w-full resize-none rounded-lg border border-[#e3beb6] bg-[#fbf9f9] p-4 text-sm outline-none focus:border-[#ee4d2d]" placeholder="Nhập mô tả chi tiết sản phẩm..." /></Field>
            ...
            <section className="rounded-xl border border-[#e3beb6] bg-white p-6 shadow-sm">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-bold">2. Bán hàng & Kho</h2><p className="mt-1 text-xs text-[#5b403b]">{hasVariants ? 'Mỗi tổ hợp biến thể sẽ có một SKU riêng.' : 'Sản phẩm sử dụng một SKU mặc định.'}</p></div><label className="flex cursor-pointer items-center gap-3 rounded-lg border border-[#e3beb6] bg-[#fff7f5] px-4 py-3"><span className="text-sm font-semibold">Sản phẩm có biến thể</span><input type="checkbox" checked={hasVariants} onChange={(event) => setHasVariants(event.target.checked)} className="peer sr-only" /><span className="relative h-6 w-11 rounded-full bg-[#c7b7b2] transition peer-checked:bg-[#ee4d2d] after:absolute after:left-1 after:top-1 after:h-4 after:w-4 after:rounded-full after:bg-white after:transition peer-checked:after:translate-x-5" /><span className="text-xs font-bold text-[#ee4d2d]">{hasVariants ? 'Bật' : 'Tắt'}</span></label></div>
              {!hasVariants ? <div className="mt-5 grid gap-4 rounded-lg border border-[#e3beb6] bg-[#fbf9f9] p-5 md:grid-cols-3"><Field label="Mã SKU tự động"><input readOnly value={defaultSku} className="h-11 w-full rounded-lg border border-[#e3beb6] bg-[#efeded] px-3 text-sm font-semibold" /></Field><Field label="Giá bán"><input type="number" min="0" value={form.price} onChange={(event) => updateForm('price', event.target.value)} className="h-11 w-full rounded-lg border border-[#e3beb6] px-3 text-right" /></Field><Field label="Tồn kho"><input type="number" min="0" value={form.stockQuantity} onChange={(event) => updateForm('stockQuantity', event.target.value)} className="h-11 w-full rounded-lg border border-[#e3beb6] px-3 text-right" /></Field></div> : null}
            </section>

            {hasVariants ? <section className="rounded-xl border border-[#e3beb6] bg-white p-6 shadow-sm"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-lg font-bold">3. Cấu hình biến thể</h2><p className="mt-1 text-xs text-[#5b403b]">Tối đa 4 thuộc tính. Mỗi giá trị đều có thể có ảnh đại diện riêng.</p></div><button type="button" onClick={addAttribute} className="flex h-10 items-center gap-2 rounded-lg border border-[#b42318] px-4 text-sm font-semibold text-[#b42318]"><SellerIcon name="add" className="text-[18px]" />Thêm thuộc tính</button></div><div className="overflow-hidden rounded-lg border border-[#e3beb6] bg-[#fbf9f9]">{attributes.map((attribute, attributeIndex) => <div key={attribute.id} className={`grid gap-4 p-4 md:grid-cols-[145px_minmax(0,1fr)_40px] ${attributeIndex < attributes.length - 1 ? 'border-b border-[#e3beb6]' : ''}`}><Field label={`Tên thuộc tính ${attributeIndex + 1}`}><input value={attribute.name} onChange={(event) => setAttributes((current) => current.map((item) => item.id === attribute.id ? { ...item, name: event.target.value } : item))} className="h-10 w-full rounded-lg border border-[#e3beb6] bg-white px-3 text-sm" /></Field><div><p className="mb-2 text-sm font-semibold">Giá trị thuộc tính <span className="font-normal text-[#8f7069]">· có thể thêm ảnh</span></p><div className="flex flex-wrap gap-2">{attribute.values.map((value) => { const imageKey = `${attribute.id}:${value}`; return <span key={value} className="inline-flex items-center gap-1 rounded-full border border-[#e3beb6] bg-white py-1.5 pl-2 pr-2 text-sm"><label className="grid h-7 w-7 cursor-pointer place-items-center overflow-hidden rounded-full border border-[#e3beb6] bg-[#fff7f5] text-[#b42318]" title={`Thêm ảnh cho ${value}`}>{variantImages[imageKey] ? <img src={variantImages[imageKey]} alt={`Biến thể ${value}`} className="h-full w-full object-cover" /> : <SellerIcon name="add_photo_alternate" className="text-[15px]" />}<input type="file" accept="image/*" className="hidden" onChange={(event) => { addVariantImage(attribute.id, value, event.target.files?.[0]); event.target.value = '' }} /></label><span className="px-1">{value}</span><button type="button" onClick={() => attribute.values.length > 1 && setAttributes((current) => current.map((item) => item.id === attribute.id ? { ...item, values: item.values.filter((entry) => entry !== value) } : item))} aria-label={`Xóa ${value}`}><SellerIcon name="close" className="text-[14px]" /></button></span> })}<button type="button" onClick={() => { setValueModalAttributeId(attribute.id); setNewValue(''); setError('') }} className="rounded-full border border-dashed border-[#ee4d2d] px-3 py-2 text-sm font-semibold text-[#ee4d2d]">+ Thêm giá trị</button></div><p className="mt-2 text-xs text-[#8f7069]">Nếu một SKU có nhiều ảnh biến thể, hệ thống ưu tiên ảnh của thuộc tính nằm trên.</p></div><button type="button" disabled={attributes.length === 1} onClick={() => setAttributes((current) => current.filter((item) => item.id !== attribute.id))} className="mt-7 grid h-9 w-9 place-items-center text-[#5b403b] disabled:opacity-30" aria-label={`Xóa thuộc tính ${attribute.name}`}><SellerIcon name="delete" className="text-[18px]" /></button></div>)}</div></section> : null}
          </div>
